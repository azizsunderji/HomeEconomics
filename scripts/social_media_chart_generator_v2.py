...
def create_exact_metro_chart(df, metro_name, metric_config, output_filename):
    ...
    fig.text(
        0.5,
        0.96,  # Moved up slightly more
        metric_name.upper(),
        ha="center",
        va="top",
        fontsize=24,  # Increased a bit more
        fontweight="bold",
        color=COLORS["black"],
    )
    
    # Add thin line above metro name - extended to chart edges
    fig.add_artist(plt.Line2D(
        [0.09, 0.90], [0.928, 0.928],  # Extended further, especially on left
    ...
    fig.text(
        0.5,
        0.910,  # Moved down a tiny bit more
        metro_display.upper(),  # Make metro name ALL CAPS
        ha="center",
        va="center",  # Center vertically
        fontsize=18,  # Keep same
        fontweight="bold",  # Make it bold
        color=COLORS["blue"],  # Use blue for emphasis
    )
    
    # Add thin line below metro name - extended to chart edges
    fig.add_artist(plt.Line2D(
        [0.09, 0.90], [0.895, 0.895],  # Extended further, especially on left
        transform=fig.transFigure,
        color=COLORS["gray"],
        linewidth=0.5,
        alpha=0.5
    ...
    # Remove y-axis title for time series chart
    ...
    # )
    ax_history.text(
        -0.05,  # Moved slightly right
        1.02,  # Lowered more to prevent overlap
    ...
    ax_history.tick_params(axis="both", colors=COLORS["black"], labelsize=10)
    ...
    legend = ax_history.legend(
        ...
        edgecolor="none",
        bbox_to_anchor=(0.98, 1.12),  # Adjusted for new title position
    )
    ...
    # Remove y-axis title for bar chart
    ...
    # )
    ax_ranking.text(
        -0.05,  # Align with Historical Trend title
        1.08,  # Just above chart
    ...
    ax_ranking.tick_params(axis="both", colors=COLORS["black"], labelsize=10)
    ...
    # Triangles - adjusted positioning
    triangle_width = 0.9
    # Add vertical offset to move triangles down
    base_y = 0  # Base position for triangles
    
    triangle_current = mpatches.Polygon(
        [(0, base_y), (triangle_width, base_y + change), (triangle_width, base_y)],
        closed=True,
        fill=False,
    ...
    triangle_historical = mpatches.Polygon(
        [(0, base_y), (triangle_width, base_y + avg_historical_change), (triangle_width, base_y)],
        closed=True,
        fill=True,
    ...
    if column_name == "AVERAGE_SALE_TO_LIST_RATIO":
        ...
        y_max_with_space = y_max * 2.0  # Increased from 1.6 to give more top space
    else:
        max_abs_change = max(abs(change), abs(avg_historical_change))
    ...
    ax_momentum.legend(
        ...
        fancybox=False,
        shadow=False,
        framealpha=0.85,  # Made background somewhat transparent
        facecolor=COLORS["background"],
        edgecolor="none",
        bbox_to_anchor=(-0.05, 1.05),  # Aligned with title on left, moved down
    )
    ...
    if len(target_row) > 0:
        ...
        ax_hist1.legend(
            ...
            facecolor=COLORS["background"],
            edgecolor="none",
            bbox_to_anchor=(1.04, 1.08),  # Moved down with the title
        )
        ...
        ax_hist1.set_xlabel(x_label, fontsize=11, fontweight="normal", color=COLORS["black"], labelpad=10)
        ax_hist1.set_ylabel("Number of Metros", fontsize=12, fontweight="normal", color=COLORS["black"], labelpad=15)
        # Add main title for BOTH histograms (only once)
        ax_hist1.text(
            -0.15,  # Moved further left
            1.08,  # Aligned with Historical Comparison title
            "Comparison Against National Data",
            transform=ax_hist1.transAxes,
            fontsize=12,  # Slightly smaller
            fontweight="bold",
            ha="left",
            va="bottom",  # Changed to bottom to match other titles
            color=COLORS["black"],
        )
        # Add subtitle with specific information
        if column_name in ["ACTIVE_LISTINGS", "WEEKS_OF_SUPPLY"]:
            subtitle_text = f"Current vs 5-Year Avg, {percentile:.0f}th %ile"
        else:
        ...
        ax_hist1.tick_params(axis="both", colors=COLORS["black"], labelsize=10)
    ...
    if len(target_row) > 0:
        ...
        if len(change_values_filtered) > 0:
            ...
            ax_hist2.legend(
                ...
                facecolor=COLORS["background"],
                edgecolor="none",
                bbox_to_anchor=(1.04, 1.18),  # Moved slightly more right
            )
            ...
            ax_hist2.set_ylabel("Number of Metros", fontsize=12, fontweight="normal", color=COLORS["black"], labelpad=15)
            # Only add subtitle (main title already shown above first histogram)
            if column_name in ["ACTIVE_LISTINGS", "WEEKS_OF_SUPPLY"]:
                subtitle_text_hist2 = f"3-Month % Change, {percentile_change:.0f}th %ile"
            else:
                subtitle_text_hist2 = f"3-Month Change, {percentile_change:.0f}th %ile"
            ax_hist2.text(
                -0.15,  # Moved further left
                1.10,  # Adjusted position
                subtitle_text_hist2,
            ...
            ax_hist2.tick_params(axis="both", colors=COLORS["black"], labelsize=10)
    ...
    # Add website URL in bottom left corner
    fig.text(
        0.09,  # Moved slightly left
        0.02,  # Near bottom
        "WWW.HOME-ECONOMICS.US",  # Small caps effect
        fontsize=8,  # Made smaller
        fontweight="bold",
        ha="left",
        va="bottom",
        color=COLORS["black"],
