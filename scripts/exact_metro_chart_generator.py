...
This replicates the EXACT Denver chart styling for any metro
"""
...
def create_exact_metro_chart(df, metro_name, metric_config, output_filename):
    """
    Create chart for a metro using EXACT Denver styling
    ...
    gs = gridspec.GridSpec(5, 1, height_ratios=[2.5, 1.5, 1.2, 2, 2], 
                          top=0.88, bottom=0.02, left=0.12, right=0.95, hspace=0.85)
    
    # Title - EXACT from Denver
    title_font = fm.FontProperties(family='ABC Oracle')
    try:
        bold_font_path = '/Users/azizsunderji/Dropbox/Home Economics/Brand assets/OracleFont/Oracle Aziz Sunderji/Desktop/ABCOracle-Bold.otf'
        if os.path.exists(bold_font_path):
            title_font = fm.FontProperties(fname=bold_font_path)
    except:
        pass
    
    fig.text(0.5, 0.985, metric_name.upper(), ha='center', va='top', 
             fontsize=32, fontproperties=title_font, color=COLORS['black'])
    fig.text(0.5, 0.965, f'{metro_display} Metro • {latest_date.strftime("%B %d, %Y")}', 
             ha='center', va='top', fontsize=20, color=COLORS['gray'])
    ...
    ax_history.fill_between(metric_data['PERIOD_END'], 0, metric_data[column_name], 
    ...
    for year in range(latest_date.year, 2016, -1):
        ...
        if len(period_data) > 0:
            ...
            ax_history.fill_between(period_data['PERIOD_END'], 0, period_data[column_name], 
                                    color='none', edgecolor=COLORS['blue'], 
                                    hatch='/////', alpha=1.0, linewidth=0)
    ...
    for year in range(latest_date.year, 2016, -1):
        ...
        value_at_date = metric_data[metric_data['PERIOD_END'] == year_end_date][column_name].iloc[0]
        ax_history.vlines(year_end_date, 0, value_at_date, color=COLORS['black'], linewidth=1.5, zorder=10)
        ax_history.plot(year_end_date, value_at_date, 'o', color=COLORS['black'], markersize=4, zorder=17)
    
    # Plot the line on top
    ax_history.plot(metric_data['PERIOD_END'], metric_data[column_name], 
                    color=COLORS['blue'], linewidth=2, zorder=15)
    
    # Format - EXACT from Denver
    ax_history.set_ylabel(f'{metric_name.title()}', fontsize=20, fontweight='normal', color=COLORS['black'], labelpad=15)
    ax_history.text(0.0, 1.12, 'Historical Trend: Weekly Data', transform=ax_history.transAxes,
                    fontsize=22, fontweight='bold', ha='left', va='top', color=COLORS['black'])
    ...
    ax_history.tick_params(axis='x', which='major', length=8, direction='out', width=1, color=COLORS['black'])
    ...
    legend = ax_history.legend([hatch_patch, line_with_dot], 
                              ['3-month Lookback', 'Current Week'],
                              loc='upper right', fontsize=14,
                              frameon=True, fancybox=False, shadow=False,
                              framealpha=1.0, facecolor=COLORS['background'], edgecolor='none',
    ...
    # Sort by values (highest to lowest) - EXACT from Denver
    ...
    for year in sorted_years:
        if year == str(latest_date.year):
            bar_colors.append(COLORS['black'])
        else:
            bar_colors.append(COLORS['blue'])
    
    bars = ax_ranking.bar(x_pos, sorted_values, color=bar_colors, edgecolor=bar_colors, 
                          linewidth=0, alpha=1.0, zorder=3)
    
    # Add value labels
    for i, (bar, val) in enumerate(zip(bars, sorted_values)):
        label = format_value(val, unit_label, decimals, is_percentage)
        ax_ranking.text(bar.get_x() + bar.get_width()/2, bar.get_height() + max(sorted_values) * 0.02,
                        label, ha='center', va='bottom', fontsize=16, 
                        color=COLORS['black'], fontweight='light')
    
    ax_ranking.set_ylabel(f'{metric_name.title()}', fontsize=20, fontweight='normal', color=COLORS['black'], labelpad=15)
    ax_ranking.text(0.0, 1.3, f'Historical Comparison: Same Week ({latest_date.strftime("%B %d")})', 
                    transform=ax_ranking.transAxes, fontsize=22, fontweight='bold', 
                    ha='left', va='top', color=COLORS['black'])
    ...
    # Create triangles - EXACT from Denver
    triangle_width = 0.9
    triangle_current = mpatches.Polygon([(0, 0), (triangle_width, change), (triangle_width, 0)], 
                                       closed=True, fill=False, edgecolor=COLORS['black'],
                                       linewidth=2, hatch='///', alpha=1.0)
    
    triangle_historical = mpatches.Polygon([(0, 0), (triangle_width, avg_historical_change), (triangle_width, 0)], 
    ...
    ax_momentum.text(0.0, 1.35, 'Momentum: 3-Month Change', transform=ax_momentum.transAxes, 
                     fontsize=22, fontweight='bold', ha='left', va='top', color=COLORS['black'])
    ...
    ax_momentum.text(triangle_width + 0.05, change, current_label, 
                     fontsize=18, va='center', color=COLORS['black'], fontweight='bold')
    ax_momentum.text(triangle_width + 0.05, avg_historical_change, historical_label, 
                     fontsize=16, va='center', color=COLORS['blue'])
    ...
    ax_momentum.legend(handles=legend_elements, loc='upper right', fontsize=14,
                      frameon=True, fancybox=False, shadow=False,
                      framealpha=1.0, facecolor=COLORS['background'], edgecolor='none',
                      bbox_to_anchor=(1, 1.25))
    
    # Set limits - EXACT from Denver
    max_abs_change = max(abs(change), abs(avg_historical_change))
    min_height = abs(latest_value) * 0.1
    if max_abs_change < min_height:
        y_max = min_height
    else:
        y_max = max_abs_change * 1.3
    ...
    # Get all metro data for histogram
    metro_data_all = df[
        (df['REGION_TYPE_ID'] == -2) & 
        (df['DURATION'] == '4 weeks') &
    ...
    # Remove outliers - EXACT from Denver
    ...
    if len(target_row) > 0:
        ...
        values_filtered = values_for_hist[(values_for_hist >= lower_bound) & (values_for_hist <= upper_bound)]
        outliers_removed = len(values_for_hist) - len(values_filtered)
        
        # Create histogram - 100 bins like Denver
        n_bins = 100
        counts, bins, patches = ax_hist1.hist(values_filtered, bins=n_bins, color=COLORS['blue'], 
                                              alpha=0.3, edgecolor=COLORS['blue'], linewidth=0.5)
        
        # Highlight target's bin
        ...
        # Add legend
        target_label = format_value(target_value_hist if normalize_for_histogram else latest_value, 
                                   normalized_unit_label if normalize_for_histogram else unit_label, 
                                   decimals, is_percentage)
        median_label = format_value(median_val, 
                                   normalized_unit_label if normalize_for_histogram else unit_label, 
                                   decimals, is_percentage)
        ...
        ax_hist1.legend(handles=legend_elements, loc='upper right', fontsize=14,
                        frameon=True, fancybox=False, shadow=False,
                        framealpha=1.0, facecolor=COLORS['background'], edgecolor='none')
    ...
    if len(target_row) > 0:
        ...
        if len(change_values) > 0 and np.std(change_values) > 0:
            ...
            change_values_filtered = change_values[(change_values >= lower_bound_change) & (change_values <= upper_bound_change)]
            outliers_removed_change = len(change_values) - len(change_values_filtered)
        else:
            change_values_filtered = change_values
            outliers_removed_change = 0
        
        if len(change_values_filtered) > 0:
            # Create histogram - 100 bins
            n_bins = 100
            counts, bins, patches = ax_hist2.hist(change_values_filtered, bins=n_bins, color=COLORS['blue'], 
                                                  alpha=0.7, edgecolor=COLORS['blue'], linewidth=0.5)
            ...
            # Format labels
            target_change_label = format_value(target_change, 
                                             normalized_unit_label if normalize_for_histogram else unit_label, 
                                             decimals, is_percentage)
            median_change_label = format_value(median_change, 
                                             normalized_unit_label if normalize_for_histogram else unit_label, 
                                             decimals, is_percentage)
            ...
            ax_hist2.legend(handles=legend_elements, loc='upper right', fontsize=14,
                            frameon=True, fancybox=False, shadow=False,
                            framealpha=1.0, facecolor=COLORS['background'], edgecolor='none')
