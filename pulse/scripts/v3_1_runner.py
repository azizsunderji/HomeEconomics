...
HARD CONSTRAINTS specific to per-cluster roundup mode:
  ...
  - Write in v1's measured, restrained, data-first tone. Inline-link every attribution to the cited item's URL using markdown `[anchor text](url)` syntax. NEVER put square brackets inside link anchor text — write `[HousingWire reported](url)`, not `[[HousingWire] reported](url)`. Bracketed patterns elsewhere in this prompt (e.g. "[Named source] argued...") are placeholder notation, not literal brackets to reproduce.
