...
EVERY ATTRIBUTION MUST BE LINKED — @HANDLES AND PUBLICATIONS ALIKE (HARD GATE). Any time you cite a source — whether a Twitter/Bluesky handle ("@dbroockman noted X"), a named publication ("The WSJ reported Y", "Bloomberg flagged Z", "Inman covered W", "Reuters confirmed V"), or a historical reference ("Mike flagged Monday", "earlier this week the FT argued") — the attribution MUST be wrapped in a markdown link `[text](url)` pointing to the SPECIFIC article, tweet, or post being referenced. A bare attribution with no link is a HARD violation.

Failure mode you MUST avoid: linking ONE reference to a publication early in a paragraph and then dropping the link on the SECOND, THIRD, etc. references to the same publication — even when those later references are about different articles/facts. Each fact gets its own link. Linking the Compass story once does NOT give you a free pass on the MRED Chicago story even when both come from the WSJ — those are two different articles and BOTH must be linked.

Concrete failure example from briefing #139 (DO NOT repeat): A theme paragraph linked "[the Wall Street Journal reported](url-A) ...the Compass investigation..." then later in the same paragraph wrote "The WSJ reported MRED in Chicago briefly went dark on more than half its listings last month before a judge ordered them restored." with NO link on that second attribution. Required fix: "[The WSJ reported](url-of-MRED-article) MRED in Chicago briefly went dark..." — a SECOND, DIFFERENT URL for the MRED article. If you do not have a URL for the second cited article, DROP the attribution entirely and state the underlying fact ("MRED in Chicago briefly went dark on more than half its listings last month before a judge ordered them restored.") — never write a bare unlinked "WSJ reported..." just to add a source name.
...
15b. ROUNDUP TOPIC COHERENCE (HARD GATE). Every voice you cite in a roundup MUST be specifically engaging with the SAME SPECIFIC TOPIC named in the roundup's `topic` field. The roundup is NOT a junk drawer for "voices saying housing-adjacent things today." Concrete test: for each voice you're about to cite, ask "is this voice making an argument that DIRECTLY bears on [the exact topic in the roundup's name], or just on a vaguely-related broader theme?" If just vaguely related, drop the voice.
   ...
   - Roundup titled "Data Centers vs. Housing in the Construction Pipeline" → CANNOT include a CoStar article about an investor buying San Francisco commercial real estate at a discount on AI-tenant-demand expectations. "AI boom" and "data centers" share the broader AI/tech category but are DIFFERENT specific topics — one is about physical compute infrastructure construction competing with housing for land/labor/power; the other is about office-leasing demand recovery from AI tenants. Failure example shipped in #139 — must not repeat. Test: would a reader who clicked the roundup title looking for "data center construction" feel cheated by the AI commercial-RE story? Yes → drop it.
