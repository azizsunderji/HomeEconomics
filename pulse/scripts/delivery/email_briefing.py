...
"""
    ...
    # ── FRONT PAGES — stylized print-edition composites ──
    # Generated by pulse/capture_frontpages.py: takes today's print PDF for
    # each paper from Freedom Forum, tilts it with a PIL perspective, and
    # composites callouts to the top headlines. JPGs are uploaded to
    # Bluehost; we embed them with a cache-buster so Gmail's image proxy
    ...
    _papers = [
        ...
        ("wt",  "The Washington Times", "https://www.washingtontimes.com"),
    ]
    html += _section_heading("On the Front Pages")
    html += _spacer(14)
    for slug, label, url in _papers:
        html += f"""<table width="100%" cellpadding="0" cellspacing="0"><tr>
<td style="padding: 0 0 10px 0;">
  <div style="font-size: 14px; color: #888; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 4px;">{label}</div>
  <a href="{url}" target="_blank"><img src="https://home-economics.us/pulse-screenshots/{slug}.jpg?v={_cb}" alt="{label} front page" width="600" style="width: 100%; max-width: 600px; height: auto; border: 1px solid #e8e8e8; border-radius: 4px; display: block;"/></a>
</td></tr></table>
"""
    html += _spacer(24)
