...
V4B_REWRITE_PREFIX = """You are REWRITING ONE existing entry of a daily housing-economics briefing so that newly attached material is integrated into it. You are not writing from scratch.
  - LONG-FORM FIRST: if the items include an author's newsletter or Substack post AND their tweets/Bluesky posts on the same point, cite the newsletter post as the source ('Conor Sen [argued](substack-url)…') and treat the social posts as reaction. Never cite a tweet in place of the same author's essay. Known identities: Bill McBride writes Calculated Risk and posts as @calculatedrisk.bsky.social on Bluesky and @BillMcBride4 on X; treat these as one voice, cite his post once, and drop the social version when it repeats the newsletter.
  - SOCIAL ATTRIBUTION: when citing a post, name the platform first: 'On X, @nickgerli1 [pushed back](url)…', 'On Bluesky, @handle [argued](url)…', 'On LinkedIn, Skylar Olsen [argued](url)…'. LinkedIn authors are display names, not @handles — never invent a handle for them. Never open with a bare @handle, and the name is never the link.
