...
def _extract_headlines(pdf_path: Path, max_headlines: int = 5) -> list[dict]:
    ...
    for s in big:
        ...
        for g in groups:
            ...
            # Vertical-gap tolerance: PDFs frequently set the next line's
            # top edge to exactly equal the previous line's bottom edge
            # (tight leading). Floating-point representation makes that
            # gap a tiny negative (vgap=-0.0), which the strict `0 <= vgap`
            # check rejected — so a 4-line single-column headline split
            # into alternating groups (briefing 2026-06-07 LA Times
            # "Establishment Democrats eke / out leads but / at crossroads").
            # Allow a small negative tolerance (-2pt) so tightly-touched
            # lines still group.
            vgap_ok = -2.0 <= vgap < line_h * 1.2
            if (same_size and vgap_ok
