...
FREE VERSION - Some features locked for subscribers only
"""
...
# Get list of change columns for merging (no filtering - Zillow data is clean)
...
if (typeof L === 'undefined') {{
    ...
    document.body.innerHTML = '<div style="padding:50px;text-align:center;">Error: Map libraries failed to load. Please refresh the page.</div>';
}}
</script>
<style>
...
/* Hide default Leaflet zoom controls since they don't work in iframe */
...
@keyframes pulse-glow {{
    0%, 100% {{
        opacity:0.5;
        transform:scale(1);
        box-shadow:0 0 30px rgba(11, 180, 255, 0.6);
    }}
    50% {{
        opacity:1;
        transform:scale(1.05);
        box-shadow:0 0 50px rgba(11, 180, 255, 1);
    }}
}}
...
.draw-icon-btn.active svg {{
    stroke:#0BB4FF;
}}
.draw-icon-btn.drawing {{
    background:#67A275;
    border-color:#67A275;
}}
.draw-icon-btn svg {{
...
<div class="modal-overlay" id="subscriptionModal">
    <div class="modal-content">
        ...
        <p class="modal-text">This feature is available to subscribers only. Subscribe to access price level comparisons, multiple time horizons, custom boundary analysis, and more advanced tools.</p>
        <div class="modal-buttons">
            <a href="https://home-economics.us/subscribe/" class="modal-button primary" target="_blank">Subscribe Now</a>
            <a href="https://home-economics.us/login/" class="modal-button secondary" target="_blank">Log In</a>
        </div>
    </div>
</div>
...
<div style="display:flex; gap:6px; margin:10px 20px 5px 20px; align-items:center;">
    <span style="font-size:9px; color:#999; margin-right:2px;">zoom</span>
    <button onclick="map.zoomOut();" class="zoom-btn">−</button>
    <button onclick="map.zoomIn();" class="zoom-btn">+</button>
</div>
<div class="legend" id="legend">
<div class="toggle-row">
    <div class="compact-toggle" onclick="showSubscriptionModal()">
        <div class="toggle-opt locked" id="toggleLevels">LEVELS 🔒</div>
...
<div class="toggle-row" id="horizonRow" style="opacity:1; pointer-events:auto;">
    <div class="compact-toggle locked" onclick="showSubscriptionModal()">
        <div class="toggle-opt locked" id="toggle3m">3M 🔒</div>
    </div>
    <div class="compact-toggle locked" onclick="showSubscriptionModal()">
        <div class="toggle-opt locked" id="toggle6m">6M 🔒</div>
    </div>
    <div class="compact-toggle" onclick="selectHorizon('1y')">
        <div class="toggle-opt active" id="toggle1y">1Y</div>
    </div>
    <div class="compact-toggle locked" onclick="showSubscriptionModal()">
        <div class="toggle-opt locked" id="toggleLong">MORE 🔒</div>
    </div>
</div>
...
<div class="toggle-row" style="align-items:center;">
    <div class="compact-toggle disabled" id="visualToggle" onclick="toggleVisualization()" style="flex:1;">
        <div class="toggle-opt active" id="toggleBubbles">BUBBLES</div>
        <div class="toggle-opt" id="toggleBoundaries">BOUNDARIES</div>
    </div>
    <button class="draw-icon-btn locked" id="drawBoundaryBtn" onclick="showSubscriptionModal()" style="opacity:0.6;" title="Draw Boundary (Subscriber Only)">
        <svg viewBox="0 0 24 24" fill="none" stroke="#666" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="M12 20h9"></path>
            <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path>
        </svg>
        <span style="position:absolute; bottom:-2px; right:-2px; font-size:10px;">🔒</span>
    </button>
</div>
<div class="gradient-bar"></div>
<div class="labels" style="font-size:8px; position:relative; margin-top:-2px;">
<span style="position:absolute; left:20%; transform:translateX(-50%);" id="q0">{quintiles.get('1y', [0,0,0,0])[0]:.0f}%</span>
<span style="position:absolute; left:40%; transform:translateX(-50%);" id="q1">{quintiles.get('1y', [0,0,0,0])[1]:.0f}%</span>
<span style="position:absolute; left:60%; transform:translateX(-50%);" id="q2">{quintiles.get('1y', [0,0,0,0])[2]:.0f}%</span>
<span style="position:absolute; left:80%; transform:translateX(-50%);" id="q3">{quintiles.get('1y', [0,0,0,0])[3]:.0f}%</span>
</div>
<div class="info-line" id="priceInfo">Year-over-Year Change</div>
<div class="info-line">Latest data: {latest_date}</div>
<div class="info-line" id="zipCount">{len(zip_data):,} ZIP codes</div>
...
// Modal functions for subscription prompts
...
// Initialize map - start at normal overview zoom
const map = L.map('map', {{
    ...
    zoomControl: false  // We'll add it manually with debugging
}});

// Manually add zoom control with event debugging
const zoomControl = L.control.zoom({{
    position: 'topleft'
}});
zoomControl.addTo(map);

// Debug zoom control clicks
console.log('[ProMap] Zoom control added to map');
map.on('zoomstart', function() {{
    console.log('[ProMap] Zoom started, current zoom:', map.getZoom());
}});
map.on('zoomend', function() {{
    console.log('[ProMap] Zoom ended, new zoom:', map.getZoom());
}});

// Add click listeners to zoom buttons for debugging
setTimeout(function() {{
    const zoomInBtn = document.querySelector('.leaflet-control-zoom-in');
    const zoomOutBtn = document.querySelector('.leaflet-control-zoom-out');

    if (zoomInBtn) {{
        console.log('[ProMap] Zoom in button found');

        // Add multiple event listeners to catch clicks
        zoomInBtn.addEventListener('click', function(e) {{
            console.log('[ProMap] Zoom in button clicked!', e);
            e.preventDefault();
            e.stopPropagation();
            map.zoomIn();
        ...
        zoomInBtn.addEventListener('mousedown', function(e) {{
            console.log('[ProMap] Zoom in mousedown!');
        }}, true);
    }} else {{
        console.error('[ProMap] Zoom in button NOT found');
    }}

    if (zoomOutBtn) {{
        console.log('[ProMap] Zoom out button found');

        // Add multiple event listeners to catch clicks
        zoomOutBtn.addEventListener('click', function(e) {{
            console.log('[ProMap] Zoom out button clicked!', e);
            e.preventDefault();
            e.stopPropagation();
            map.zoomOut();
        }}, true);  // Use capture phase
        ...
        zoomOutBtn.addEventListener('mousedown', function(e) {{
            console.log('[ProMap] Zoom out mousedown!');
        }}, true);
    }} else {{
        console.error('[ProMap] Zoom out button NOT found');
    }}

    // Also add a global click listener to debug what's happening
    document.addEventListener('click', function(e) {{
        console.log('[ProMap] Click detected on:', e.target);
    }}, true);
}}, 1000);
...
function updateLegend(quintiles, zipCount, minPop, maxPop, isSmallSample = false) {{
    ...
    if (isLocalMode) {{
        document.getElementById('zipCount').textContent = `${{zipCount}} ZIP code${{zipCount === 1 ? '' : 's'}} in view`;
        ...
        document.getElementById('zipCount').textContent = '26,301 ZIP codes';
        document.getElementById('popRange').style.display = 'none';
        document.getElementById('sizeNote').innerHTML = 'Bubble size reflects population<br>Zoom in for details';
    }}
}}
...
function toggleView() {{
    console.log('[ProMap] toggleView called');
...
function toggleDataMode() {{
    console.log('[ProMap] toggleDataMode called');
...
async function toggleVisualization() {{
    ...
    if (visualToggle.classList.contains('disabled')) {{
        console.log('[ProMap] Cannot toggle to boundary view - zoom level too low (need zoom >= 6)');
        return;
    }}

    console.log('[ProMap] Toggling boundary view, current state:', isBoundaryView);
...
const tutorialSteps = [
    ...
    {{
        ...
        action: function() {{
            // Switch to changes mode if currently in price mode
            if (dataMode === 'price') {{
                toggleDataMode();
            }}
        }}
    ...
    {{
        title: "Time Periods",
        text: "When viewing changes, select different time horizons: <span class='btn-ref'>3M</span>, <span class='btn-ref'>6M</span>, <span class='btn-ref'>1Y</span>, or click <span class='btn-ref'>MORE</span> for longer periods.",
        spotlight: "#horizonRow",
        action: function() {{
            // Ensure we're in changes mode
            if (dataMode === 'price') {{
                toggleDataMode();
            }}
        }}
    ...
    {{
        ...
        action: function() {{
            // Switch to local mode to demonstrate (we're already zoomed into NYC)
            if (!isLocalMode) {{
                toggleView();
            }}
        }}
    ...
    {{
        ...
        text: "Click the pencil icon to draw a custom area. The map will show only ZIP codes within your boundary and recalculate quintiles for that area.",
        spotlight: ".draw-icon-btn"
    }},
    {{
        title: "Search Locations",
        text: "Search for any ZIP code or place name to jump directly to that location.",
        spotlight: ".search-wrapper",
        action: function() {{
            // Clear any drawn boundary so search results are visible
            if (drawnBoundary) {{
                clearDrawnBoundary();
            }}
            // Switch back to bubble view for better search experience
            if (isBoundaryView) {{
                toggleVisualization();
            }}
        }}
...
# Write HTML file
output_file = 'output/ProMap_free.html'
...
print("   • 1-year price changes (CHANGES mode only)")
...
print("\n🔒 LOCKED Features (require subscription):")
print("   • Price LEVELS toggle")
print("   • Multiple time horizons (3M, 6M, 3Y, 5Y, 10Y, 15Y)")
print("   • Custom boundary drawing")
print("   • Modal prompts users to subscribe or login for premium features")