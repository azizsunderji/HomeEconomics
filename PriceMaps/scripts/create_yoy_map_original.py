...
if date_columns:
    ...
    for col in date_columns:
        if '/' in col:
            parts = col.split('/')
            if len(parts) == 3:
                col_month = int(parts[0])
                col_year = int('20' + parts[2]) if len(parts[2]) == 2 else int(parts[2])
                if col_month == latest_month and col_year == target_year:
                    year_ago_date = col
                    break
        elif '-' in col:
            parts = col.split('-')
            if len(parts) == 3:
                col_year = int(parts[0])
                col_month = int(parts[1])
                if col_month == latest_month and col_year == target_year:
                    year_ago_date = col
                    break

    # If no exact match, find closest date from previous year
    if not year_ago_date:
            ...
            if '/' in col:
                ...
                if len(parts) == 3:
                    ...
                    if col_year == target_year:
                        year_ago_candidates.append(col)
            elif '-' in col:
                parts = col.split('-')
                if len(parts) == 3:
                    col_year = int(parts[0])
        ...
        if year_ago_candidates:
            year_ago_date = year_ago_candidates[-1]
...
# Write HTML file
output_file = script_dir / 'output' / 'us_yoy_price_map_with_search.html'
